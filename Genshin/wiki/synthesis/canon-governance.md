...
1. **Nguồn sơ cấp trực tiếp trong game (Primary In-game Sources):** Hội thoại nhiệm vụ (Quest dialogue), mô tả vật phẩm/vũ khí ([weapon_stories](sources/weapon_stories/)), mô tả thánh di vật ([artifact_descriptions](sources/artifact_descriptions/)), sách trong game ([book_texts](sources/book_texts/)), và câu chuyện/lời thoại nhân vật ([voice_lines](sources/voice_lines/)). Đây là bộ xương sống bất biến.
...
5. **Headcanon / AU:** Các phác thảo tâm lý tự bổ sung hoặc giả thiết thế giới song song trong các tài liệu phân tích tính cách nhân vật (đặc biệt là các mục từ 3-8 trong hồ sơ [character_intelligence](entities/character_intelligence/)).
